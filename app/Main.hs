...
import qualified Data.GraphViz.Attributes as ATT
import           Data.GraphViz.Attributes.Complete as ATTC
import           Diagrams.Backend.Rasterific.CmdLine as R
import           Diagrams.Prelude as D
import           Diagrams.TwoD.GraphViz
import qualified Data.Text.Lazy as L
import           Diagrams.Backend.Cairo.CmdLine as C
...
generateNetworkGraph2 :: Event ()
generateNetworkGraph2 = do
  gen <- gets randomNumGen
  addresses <- liftIO $ replicateM 6 $ genAddress gen
  let vertexList = [0..5]
          -- Label the vertices with their addresses
      labelledNodes = zip vertexList addresses

      labelledEdges = [(0,1,()), (0,5,()), (1,4,()),(4,1,()), (2,3,()) ,(2,5,()),  (3,4,())]
      g = GIG.mkGraph labelledNodes labelledEdges :: Gr NodeAddress ()
  modify $ \lnst -> lnst { networkGraph = g }
...
main = do
  ...
  let myColorCL :: ColorList
      myColorCL = c $ (ATTC.RGB 127 108 138)
        where c rgb = toColorList [rgb]

  let numiter = 10
  (finalState, stats) <- execRWST (lightningSim numiter) lnconfig initialLNState
  let gr = networkGraph $ finalState

  let params :: GraphvizParams Int v e () v
      params = defaultDiaParams
               { fmtEdge = const [arrowTo noArrow],fmtNode = fn,globalAttributes = ga}
      fn(_,_) = [style dotted]
      ga = [ GraphAttrs [ RankDir FromLeft, BgColor [toWColor Red]], NodeAttrs [ shape BoxShape, FillColor (myColorCL), style filled]]
  hex' <- layoutGraph' params Dot gr
  let hexDrawing :: Diagram R.B
      hexDrawing = drawGraph
                     (const $ place (circle 19))
                     (\_ _ _ _ _ p -> stroke p)
                     hex'
                     -- to try other backend than rasterific svg or 
